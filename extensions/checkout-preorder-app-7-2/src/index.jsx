...
function App() {
  ...
  useEffect(() => {
    if (showError) {
      ...
      console.log('log 1) error'); // no error
  ...
  if (loading) {
    return (
      <BlockStack  >
      <Text size="small" >Item dispatched by 
        <Text emphasis="italic"> DD/MM</Text>
      </Text>
    </BlockStack>
    );
  }

  console.log('log 2) products.length  = '+ products.length ); // 0 0 when metafields in the query, 0 5 when MF removed
  // If product variants can't be loaded, then show nothing
  if (!loading && products.length === 0) {
    console.log('log 4) product.length = 0, so return');
    return null;
  }

  /// need to grab the items in the cart now
  ...
  // const {  title, id, variants} = productsOnOffer[0];
